...
class MySql(AgentCheck):
    ...
    def __init__(self, name, init_config, instances):
        ...
        self._query_activity = MySQLActivity(
            self, self._config, self._get_connection_args()
        )
    ...
    def _check_performance_schema_enabled(self, db):
        if self.performance_schema_enabled is None:
            with closing(db.cursor()) as cursor:
                ...
                self.performance_schema_enabled = self._get_variable_enabled(
    ...
    def _check_events_wait_current_enabled(self, db):
        if not self._config.dbm_enabled or not self._config.activity_config.get(
            "enabled", True
        ):
            self.log.debug(
                "skipping _check_events_wait_current_enabled because dbm activity collection is not enabled"
            )
            return
        if not self._check_performance_schema_enabled(db):
            self.log.debug(
                "`performance_schema` is required to enable `events_waits_current`"
            )
            return
        if self.events_wait_current_enabled is None:
            with closing(db.cursor()) as cursor:
                ...
                results = dict(cursor.fetchall())
                self.events_wait_current_enabled = self._get_variable_enabled(
                    results, "events_waits_current"
                )
    ...
    def _collect_metrics(self, db, tags):
        ...
        if not is_affirmative(
            ...
            self.innodb_stats.process_innodb_stats(
                results, self._config.options, metrics
            )
        ...
        try:
            ...
            results["Key_buffer_bytes_used"] = (
                collect_scalar("Key_blocks_used", results) * key_cache_block_size
            )
        ...
        if is_affirmative(
            self._config.options.get("extra_status_metrics", self._config.dbm_enabled)
        ):
        ...
        self.log.warning(
            "Hello world %s",
            is_affirmative(
                self._config.options.get("extra_performance_metrics", False)
            ),
        )
        ...
        if (
            ...
            ] = self._get_query_exec_time_95th_us(db)
        ...
        if (
            is_affirmative(self._config.options.get("table_rows_stats_metrics", False))
            and self.userstat_enabled
        ):
        ...
        if is_affirmative(self._config.options.get("system_table_size_metrics", False)):
            # report size of tables in MiB to Datadog
            (table_index_size, table_data_size) = self._query_size_per_table(
                db, system_tables=True
            )
            results["information_table_index_size"] = table_index_size
            results["information_table_data_size"] = table_data_size
            metrics.update(TABLE_VARS)
        ...
        if is_affirmative(
            self._config.options.get("replication", self._config.dbm_enabled)
        ):
            if self.performance_schema_enabled and self._is_group_replication_active(
                db
            ):
        ...
        if len(self._config.additional_status) > 0:
            ...
            for status_dict in self._config.additional_status:
                ...
                if status_name in metrics.keys():
                    ...
                    additional_status_dict[status_dict["name"]] = (
                        status_dict["metric_name"],
                        status_dict["type"],
                    )
        ...
        if len(self._config.additional_variable) > 0:
            ...
            for variable_dict in self._config.additional_variable:
                ...
                if variable_name in metrics.keys():
                    ...
                    additional_variable_dict[variable_name] = (
                        variable_metric,
                        variable_dict["type"],
                    )
        ...
        if isinstance(self._config.queries, list):
            ...
            if len(self._config.queries) > self._config.max_custom_queries:
                self.warning(
                    ...
                    self._config.max_custom_queries,
                )
    ...
    def _collect_replication_metrics(self, db, results, above_560):
        ...
        results.update(
            self._get_replica_stats(db, self.is_mariadb, replication_channel)
        )
        nonblocking = is_affirmative(
            self._config.options.get("replication_non_blocking_status", False)
        )
        ...
        try:
            with closing(db.cursor()) as cursor:
                ...
                if replica_results is None or len(replica_results) < 3:
                    ...
                    status = (
                        self.OK if replica_results[1] == "ONLINE" else self.CRITICAL
                    )
                    additional_tags = [
                    ...
                    self.gauge(
                        ...
                        1,
                        tags=additional_tags + self._config.tags,
                    )
                ...
                self._submit_metrics(
                    GROUP_REPLICATION_VARS,
                    results,
                    self._config.tags + ["channel_name:{}".format(r[0])],
                )

                return GROUP_REPLICATION_VARS
        except Exception as e:
            self.warning(
                "Internal error happened during the group replication check: %s", e
            )
            return {}
    ...
    def _check_replication_status(self, results):
        ...
        if replica_io_running:
            replica_io_running = any(
                v.lower().strip() == "yes" for v in itervalues(replica_io_running)
            )
        if replica_sql_running:
            replica_sql_running = any(
                v.lower().strip() == "yes" for v in itervalues(replica_sql_running)
            )
        ...
        if self._is_source_host(replicas, results):
            ...
            self._submit_replication_status(
                source_repl_running_status, ["replication_mode:source"]
            )
        ...
        if self._is_replica_host(replicas, results):
            if not (replica_io_running is None and replica_sql_running is None):
                if not replica_io_running and not replica_sql_running:
                    self.log.debug(
                        "Replica_IO_Running and Replica_SQL_Running are not ok"
                    )
                    replica_running_status = AgentCheck.CRITICAL
                elif not replica_io_running or not replica_sql_running:
                    self.log.debug(
                        "Either Replica_IO_Running or Replica_SQL_Running are not ok"
                    )
                ...
                self._submit_replication_status(
                    replica_running_status, ["replication_mode:replica"]
                )
    ...
    def _is_source_host(self, replicas, results):
        ...
        source_host = collect_string("Master_Host", results) or collect_string(
            "Source_Host", results
        )
    ...
    def _is_replica_host(self, replicas, results):
        return collect_string("Master_Host", results) or collect_string(
            "Source_Host", results
        )
        ...
        for variable, metric in iteritems(variables):
            if isinstance(metric, list):
                for m in metric:
                    ...
                    self.__submit_metric(
                        metric_name, metric_type, variable, db_results, tags
                    )
            else:
            ...
            with closing(db.cursor()) as cursor:
                ...
                if result is not None:
                    for field, metric in list(iteritems(field_metric_map)):
                        ...
                        try:
                            col_idx = [d[0].lower() for d in cursor.description].index(
                                field.lower()
                            )
                            ...
                            self.log.exception(
                                "Cannot find %s in the columns %s",
                                field,
                                cursor.description,
                            )
        except Exception:
    ...
    def _get_runtime_aurora_tags(self, db):
        ...
        try:
            ...
            self.log.warning(
                "Error occurred while fetching Aurora runtime tags: %s",
    ...
    def _collect_system_metrics(self, host, db, tags):
        ...
        if pid:
            ...
            try:
                if PSUTIL_AVAILABLE:
                    self.log.debug(
                        "psutil is available, attempting to collect mysql.performance.* metrics"
                    )
                    ...
                    self.log.debug(
                        "psutil is not available, will not collect mysql.performance.* metrics"
                    )
            except Exception:
                self.warning(
    ...
    def _get_replica_stats(self, db, is_mariadb, replication_channel):
        ...
        try:
            with closing(db.cursor(pymysql.cursors.DictCursor)) as cursor:
                ...
                cursor.execute(
                    show_replica_status_query(
                        self.version, is_mariadb, replication_channel
                    )
                )
                ...
                for replica_result in results:
                    ...
                    channel = (
                        replication_channel
                        or replica_result.get("Channel_Name")
                        or "default"
                    )
    ...
    def _get_query_exec_time_95th_us(self, db):
        ...
        try:
            ...
            self.warning(
                "95th percentile performance metrics unavailable at this time: %s", e
            )
            return None
        ...
        try:
            with closing(db.cursor()) as cursor:
                ...
                for row in cursor.fetchall():
                    ...
                    # set the tag as the dictionary key
                    table_index_size[
                        "schema:{},table:{}".format(table_schema, table_name)
                    ] = index_size
                    table_data_size[
                        "schema:{},table:{}".format(table_schema, table_name)
                    ] = data_size
    ...
    def _query_size_per_schema(self, db):
        ...
        try:
            ...
            self.warning(
                "Avg exec time performance metrics unavailable at this time: %s", e
            )
    ...
    def _query_rows_stats_per_table(self, db):
        try:
            with closing(db.cursor()) as cursor:
                ...
                if cursor.rowcount < 1:
                    self.warning(
                        "Failed to fetch records from the tables rows stats 'tables' table."
                    )
                    return None
    ...
    def _compute_synthetic_results(self, results):
        if (
            ("Qcache_hits" in results)
            and ("Qcache_inserts" in results)
            and ("Qcache_not_cached" in results)
        ):
